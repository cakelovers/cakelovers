    ...
    <div className="flex min-h-dvh flex-col bg-background">
      ...
      <section className="relative aspect-[11/6] w-full overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-end bg-gradient-to-t from-primary/80 via-primary/10 to-transparent p-5 sm:p-12">
          <div className="mx-auto flex w-full max-w-5xl flex-col items-start gap-4 sm:gap-6">
            <div>
              <h1 className="font-heading text-2xl leading-snug font-bold text-primary-foreground sm:text-5xl">
                주문제작 케이크를
                <br />
                더 편하게.
              </h1>
              <p className="mt-2 max-w-xs text-xs leading-relaxed text-primary-foreground/85 sm:mt-3 sm:text-sm">
                원하는 모습을 적기만 하면,
                <br />
                나머지는 저희가 준비할게요.
              </p>
            ...
            {ORDER_STORY.map((item) => (
              <div key={item.step} className="flex flex-col gap-3 bg-card p-6">
                {item.image ? (
                  ...
                  <div className="flex aspect-[4/3] w-full items-center justify-center rounded-lg border border-dashed bg-muted/40 p-4 text-center text-xs text-muted-foreground">
                    AI 생성 이미지 준비 중
                  </div>
                )}
                <div>
