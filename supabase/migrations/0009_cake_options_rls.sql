...
-- presets only for a store they belong to. No anon policy — the public
-- order flow (the cake-options route and order submission) reads
-- through the service-role client, which bypasses RLS, same as pickup
-- and payment settings. No DELETE policy — presets are disabled via
-- is_enabled, never removed, so an id already referenced by a past
-- order keeps resolving.
--
